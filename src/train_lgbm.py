...
def time_series_split_indices(n: int, n_splits: int = 3, val_len: int = 24 * 7) -> List[Tuple[np.ndarray, np.ndarray]]:
    idx = np.arange(n)
...
def train_and_predict(train_csv: Path, test_csv: Path, out_csv: Path, cv_folds: int = 0, cv_val_len: int = 24*7, cv_verbose: bool = True) -> None:
    ...
    # Winsorize targets to reduce outlier impact
    ...
    # Match sample_submission id format: 'YYYY-MM-DD HH' (no minutes/seconds)
    preds = {
        "id": test_feat["id"].dt.strftime("%Y-%m-%d %H")
    }
    ...
    for target in TARGET_COLUMNS:
        ...
        if cv_folds and cv_folds > 0:
            ...
            model.fit(
                X_valid,
                y_valid,
                eval_set=[(X_valid.tail(cv_val_len), y_valid[-cv_val_len:])],
                eval_metric="l1",
                callbacks=[early_stopping(50), log_evaluation(0)],
            )
        else:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train LightGBM and predict test set")
    parser.add_argument(
        "--data-dir",
        type=str,
